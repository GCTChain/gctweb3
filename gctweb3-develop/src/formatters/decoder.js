...
const TestnetNetworkByte = '5C';

class Decoder {
  static toEcoAddress(hexAddress, isMainnet = false) {
    ...
    let gctAdress = hexAddress;
    // Add network byte
    if (isMainnet) {
      gctAdress = MainnetNetworkByte + gctAdress;
    } else {
      gctAdress = TestnetNetworkByte + gctAdress;
    }

    const gctAdressBuffer = Buffer.from(gctAdress, 'hex');
    // Double SHA256 hash
    const hash1 = createHash('sha256').update(gctAdressBuffer).digest('Hex');
    const hash1Buffer = Buffer.from(hash1, 'hex');
    const hash2 = createHash('sha256').update(hash1Buffer).digest('Hex');

    // get first 4 bytes
    gctAdress += hash2.slice(0, 8);

    // base58 encode
    const address = bs58.encode(Buffer.from(gctAdress, 'hex'));
    return address;
  }
